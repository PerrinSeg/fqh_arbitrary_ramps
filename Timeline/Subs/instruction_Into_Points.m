...
%         disp('AddInterpolatedRampUsingFile')
    ...
    elseif contains(lower(name_fun), lower('AddTunnelRamp'))
        % analogdata.AddTunnelRamp(conversion_coeffs,
        %       start_tunneling, stop_tunneling, t_start, t_stop, 
        %       voltage_offset, calib_volt, calib_depth, dat_chan)
        % { 
        ...
        %     }
        % }
        % disp('REACHED AddTunnelRamp')
        ncoeffs = 11;
        coeff_array = zeros(1,ncoeffs);
        for i = 1:ncoeffs
        ...
        t_start = eval(arguments{5});
        t_stop = eval(arguments{6});
        voltage_offset = eval(arguments{7});
        calib_volt = eval(arguments{8});
        calib_depth = eval(arguments{9});
        
        depth_from_tunnel = @(a, j_er) a(1) + a(2)*exp(-j_er/a(3)) + a(4)*exp(-j_er/a(5)) + a(6)*exp(-j_er/a(7)) + a(8)*exp(-j_er/a(9)) + a(10)*exp(-j_er/a(11));

        slope = (j_stop - j_start) / (t_stop - t_start);
        N = 100;
        time = linspace(t_start, t_stop, N);
    ...
    elseif contains(lower(name_fun), lower('AddTunnelGaugeRamp'))
        ...
        %       start_tunneling, stop_tunneling, t_start, t_stop,
        %       calib_volt, calib_depth, dat_chan)
        % { 
        %       double slope = (stop_tunneling - start_tunneling) / ((t_stop - t_start) * SMP_CLK / BPW / 1000);
        %       double x;
        %       double y;
        ...
        %       }
        % }
        % disp('REACHED AddTunnelGaugeRamp')
        % arguments
        % arguments{2}
        % arguments{2}{:}
        ncoeffs = 4;
        ...
        t_start = eval(arguments{5});
        t_stop = eval(arguments{6});
        calib_volt = eval(arguments{7});
        calib_depth = eval(arguments{8});
        
        gauge_depth_from_tunnel = @(a, j_er) a(1)*j_er + a(2)*j_er.^2 + a(3)*j_er.^3 + a(4)*j_er.^4;
