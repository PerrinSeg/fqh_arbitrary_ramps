...
quic_ramp_dur = 0
quic_grad_level_init = 0
quic_grad_level_final = 0
quad_ramp_dur = 0
quad_grad_level_init = 0
quad_grad_level_final = 0
lattice2_ramp_dur = 0
...
walls1_volt = 3.1
use_gauge = 0
gauge_power_init = 2.88
gauge_power_final = 2.7
gauge_power_ramp_dur = 0
gauge_freq_khz = 0.87
flux = 0
evolution_dur = 0
is_return = 0
...
If (quic_grad_level_init > 0) Then
...
If (quad_grad_level_init > 0) Then
...
Dim lattice1_lower_start_time As Double = quad_turnon_end_time
Dim lattice1_lower_end_time As Double = lattice1_lower_start_time + lattice1_ramp_dur

'() cherp freq to enable dynamics
Dim freq_cherp_start_time As Double = lattice1_lower_end_time
...
If (use_gauge > 0) Then 
    raman_rampup_end_time = raman_rampup_start_time + gauge_ramp_dur
Else
    raman_rampup_end_time = raman_rampup_start_time
End If

'() Lower quad gradient
Dim quad_lower_start_time As Double = raman_rampup_end_time
Dim quad_lower_end_time As Double = quad_lower_start_time + quad_ramp_dur

'() Decrease Raman power to equal tunneling
Dim raman_decrease_start_time As Double = quad_lower_end_time
Dim raman_decrease_end_time As Double = raman_decrease_start_time + gauge_power_ramp_dur

'() Hold time
Dim hold_start_time As Double = raman_decrease_end_time
Dim hold_end_time As Double = hold_start_time + evolution_dur

'() RETURN: increase Raman power
Dim raman_increase_start_time As Double = hold_end_time
Dim raman_increase_end_time As Double
If (is_return > 0) Then 
    raman_increase_end_time = raman_increase_start_time + use_gauge * gauge_power_ramp_dur
Else
    raman_increase_end_time = raman_increase_start_time
End If

'() RETURN: raise quad gradient
Dim quad_raise_start_time As Double = raman_increase_end_time
...
Dim raman_rampdown_start_time As Double = quad_raise_end_time
Dim raman_rampdown_end_time As Double 
If (is_return > 0) Then 
    raman_rampdown_end_time = raman_rampdown_start_time + use_gauge * gauge_ramp_dur
Else
    raman_rampdown_end_time = raman_rampdown_start_time
End If

'() RETURN: raise 2D1 lattice depth
Dim lattice1_raise_start_time As Double = raman_rampdown_end_time
...
Dim quad_turnoff_start_time As Double = lattice1_raise_end_time
Dim quad_turnoff_end_time As Double 
If (is_return > 0) Then 
    quad_turnoff_end_time = quad_turnoff_start_time + coil_ramp_dur
Else 
...
'Dim cutting1_turnon2_start_time As Double = quic_turnoff_end_time
...
If (lattice2_ramp_dur > 0) Then
    ...
    Else
        ...
        analogdata.AddStep(lattice2_low_volt, lattice2_raise_start_time, lattice2_freeze_start_time-1, lattice2D765_power2)
        analogdata.AddSmoothRamp(lattice2_low_volt, lattice2_max_volt, lattice2_freeze_start_time-1, lattice2_freeze_start_time, lattice2D765_power2)
    End If
Else
...
If (lattice1_ramp_dur > 0) Then
    ...
    Else
        'quench to deep lattice
        analogdata.AddStep(lattice1_low_volt, lattice1_lower_end_time, twod1_rampdown2_start_time-1, lattice2D765_power)
        analogdata.AddSmoothRamp(lattice1_low_volt, lattice1_max_volt, twod1_rampdown2_start_time-1, twod1_rampdown2_start_time, lattice2D765_power)
    End If
Else
...
If (quic_grad_level_init > 0) Then 
    'turn on quic
    analogdata2.AddSmoothRamp(0, quic_grad_level_init * ps5_scaler, quic_turnon_start_time, quic_turnon_end_time, ps5_ao)
    analogdata2.AddStep(quic_grad_level_init * ps5_scaler, quic_turnon_end_time, quic_lower_start_time, ps5_ao)
    'lower quic
    analogdata2.AddSmoothRamp(quic_grad_level_init * ps5_scaler, quic_grad_level_final * ps5_scaler, quic_lower_start_time, quic_lower_end_time, ps5_ao)
    analogdata2.AddStep(quic_grad_level_final * ps5_scaler, quic_lower_end_time, quic_raise_start_time, ps5_ao)
    If (is_return > 0) Then
        'raise quic
        analogdata2.AddSmoothRamp(quic_grad_level_final * ps5_scaler, quic_grad_level_init * ps5_scaler, quic_raise_start_time, quic_raise_end_time, ps5_ao)
        analogdata2.AddStep(quic_grad_level_init * ps5_scaler, quic_raise_end_time, quic_turnoff_start_time, ps5_ao)
        'turn off
        analogdata2.AddSmoothRamp(quic_grad_level_init * ps5_scaler, 0, quic_turnoff_start_time, quic_turnoff_end_time, ps5_ao)
    Else
        analogdata2.AddStep(quic_grad_level_final * ps5_scaler, quic_raise_start_time, grad_turnoff_start_time, ps5_ao)
        'turn off 
        analogdata2.AddSmoothRamp(quic_grad_level_final * ps5_scaler, 0, grad_turnoff_start_time, grad_turnoff_end_time, ps5_ao)
    End If
    'keep off 
...
If (quad_grad_level_init > 0) Then
    'turn on quad
    analogdata2.AddSmoothRamp(0, quad_grad_level_init, quad_turnon_start_time, quad_turnon_end_time, ps8_ao)
    analogdata2.AddStep(quad_grad_level_init, quad_turnon_end_time, quad_lower_start_time, ps8_ao)
    'lower quad
    analogdata2.AddSmoothRamp(quad_grad_level_init, quad_grad_level_final, quad_lower_start_time, quad_lower_end_time, ps8_ao)
    analogdata2.AddStep(quad_grad_level_final, quad_lower_end_time, quad_raise_start_time, ps8_ao)
    If (is_return > 0) Then
        'raise quad
        analogdata2.AddSmoothRamp(quad_grad_level_final, quad_grad_level_init, quad_raise_start_time, quad_raise_end_time, ps8_ao)
        analogdata2.AddStep(quad_grad_level_init, quad_raise_end_time, quad_turnoff_start_time, ps8_ao)
        'turn off
        analogdata2.AddSmoothRamp(quad_grad_level_init, 0, quad_turnoff_start_time, quad_turnoff_end_time, ps8_ao)
    Else
        analogdata2.AddStep(quad_grad_level_final, quad_raise_start_time, grad_turnoff_start_time, ps8_ao)
        'turn off 
        analogdata2.AddSmoothRamp(quad_grad_level_final, 0, grad_turnoff_start_time, grad_turnoff_end_time, ps8_ao)
    End If
    'keep off 
...
If (use_gauge > 0) Then
    ...
    'analogdata2.AddRamp(0, gauge1_pzY, piezo_rampup_time, piezo_ready_time, gauge1_PZTy)
    ...
    'analogdata2.AddRamp(0, gauge2_pzY, piezo_rampup_time, piezo_ready_time, gauge2_PZTy)
    ...
    digitaldata2.AddPulse(gauge_shutter, raman_rampup_start_time - 5, raman_rampdown_end_time + 10)
    digitaldata2.AddPulse(gauge_ttl, raman_rampup_start_time - 10, raman_rampdown_end_time)
	analogdata2.AddStep(4.99, raman_rampup_start_time, raman_rampdown_end_time, gauge2_power)

	'Ramp Raman power
	analogdata2.AddStep(0.6, 0, raman_rampup_start_time, gauge1_power)
	analogdata2.AddSmoothRamp(0.6, gauge_power_init, raman_rampup_start_time, raman_rampup_end_time, gauge1_power)
	analogdata2.AddStep(gauge_power_init, raman_rampup_end_time, raman_decrease_start_time, gauge1_power)
    'ramp to equal tunneling
    analogdata2.AddSmoothRamp(gauge_power_init, gauge_power_final, raman_decrease_start_time, raman_decrease_end_time, gauge1_power)
    analogdata2.AddStep(gauge_power_final, raman_decrease_end_time, raman_increase_start_time, gauge1_power)
    If (is_return > 0) Then 
        analogdata2.AddSmoothRamp(gauge_power_final, gauge_power_init, raman_increase_start_time, raman_increase_end_time, gauge1_power)
        analogdata2.AddStep(gauge_power_init, raman_increase_end_time, raman_rampdown_start_time, gauge1_power)
        analogdata2.AddSmoothRamp(gauge_power_init, 0.6, raman_rampdown_start_time, raman_rampdown_end_time, gauge1_power)
    Else 
        analogdata2.AddStep(gauge_power_final, raman_increase_start_time, raman_rampdown_start_time, gauge1_power)
        analogdata2.AddSmoothRamp(gauge_power_final, 0.6, raman_rampdown_start_time, raman_rampdown_end_time, gauge1_power)
    End If

	'Raman-assisted tunneling
	'analogdata2.AddSmoothRamp(0, gauge_freq_volt, freq_cherp_start_time, freq_cherp_end_time, gauge2_rf_fm)
...
Dim tracking_end_time As Double = Me.AddTrackingSequence2(pinning_end_time, cp, analogdata, analogdata2, digitaldata, digitaldata2, gpib, Hermes, dds, True)
Dim last_time As Double = tracking_end_time
